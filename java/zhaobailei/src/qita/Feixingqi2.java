...
public class Feixingqi2 {
	static  int yihao = 20;
	static 	int erhao = 20;
	static 	 int sanhao = 20;
	static  int sihao = 20;
	static int yihaosheng = 0;
	 static int erhaosheng = 0;
	 static  int sanhaosheng = 0;
	 static int sihaosheng = 0;
		...
		for (int i = 0; i < 100; i++) {
			...
			for (int u = 4; u < 8; u++) {
				if (u % 4 == 0) {
					...
					zifu("----------------------------\r" + "玩家三号请按数字键3摇动筛子\n"
							+ "按错者跳过本回合");
					int san = Anjian();
					if (san == 3) {
						int a = Shuiji(0);
						sanhaosheng = sanhao - a;

						if (sanhaosheng < 0) {
							sanhaosheng = sanhaosheng * (-1);
							sanhao = sanhaosheng;
							zifu("三号玩家前进了" + a + "由于步数超出终点，");
							zifu("那么当前步行数*（-1）为正数后继续游戏");
							zifu("剩余步数为" + sanhaosheng + "步");
						} else if (sanhaosheng == 0) {
							zifu("三号玩家前进了" + a + "最先到达终点，三号" + "玩家获得最先的胜利，游戏结束");
							break;
						} else if (sanhaosheng > 0) {
							zifu("三号玩家前进了" + a + "距离终点还剩余" + sanhaosheng + "步");
							sanhao = sanhaosheng;
						}
					} else {
						zifu("由于玩家三摇错筛子\n" + "玩家三结束本回合");
					...
					if (si == 4) {
						...
						if (sihaosheng < 0) {
							sihaosheng = sihaosheng * (-1);
							sihao = sihaosheng;
							zifu("四号玩家前进了" + a + "由于步数超出终点，");
							zifu("那么当前步行数*（-1）为正数后继续游戏");
							zifu("剩余步数为" + sihaosheng + "步");
						} else if (sihaosheng == 0) {
							zifu("四号玩家前进了" + a + "最先到达终点，四号" + "玩家获得最先的胜利，游戏结束");
							break;
						} else if (sihaosheng > 0) {
							zifu("四号玩家前进了" + a + "距离终点还剩余" + sihaosheng + "步");
							sihao = sihaosheng;
						}
					} else {
						zifu("由于玩家四摇错筛子\n" + "玩家四结束本回合");
